...
void ggml_compute_forward_lightning_indexer(
    ...
    // per-thread scratch for K converted to float; stride matches the work-size
    // reserved in ggml-cpu.c (n_embd + CACHE_LINE_SIZE_F32) floats per thread.
    float * src1_row_f32 = (float *) params->wdata + ith*(n_embd + CACHE_LINE_SIZE_F32);

    // rows (KV positions) per thread
    const int dr  = (nr + nth - 1)/nth;
    const int ir0 = dr*ith;
    const int ir1 = MIN(ir0 + dr, nr);
        ...
        for (int i_batch = 0; i_batch < n_batch; ++i_batch) {
            for (int i_kv = ir0; i_kv < ir1; ++i_kv) {
                ...
                if (k_to_float) {
                    ...
                    k_row_f32 = src1_row_f32;
                } else {
                    k_row_f32 = (float *) src1_row;
                }
                const float * src2_row = (const float *) ((const char *) src2->data + i_batch*nb21 + i_stream*nb23);
                ...
                for (int i_head = 0; i_head < n_head; ++i_head) {
                    ...
                    float qk = 0.0f;
                    const float * src0_row = (const float *) ((const char *) src0->data + i_head*nb01 + i_batch*nb02 + i_stream*nb03);
                    ggml_vec_dot_f32(n_embd, &qk, 0, src0_row, 0, k_row_f32, 0, 1);
                    qk *= scale_embd;
                    // ReLU and weights
